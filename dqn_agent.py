...
E = 1e-4                # small constant to add to td_error
...
class Agent():
    ...
    def learn(self, experiences, gamma):
        ...
        """
        states, actions, rewards, next_states, abs_td_errors, dones = experiences
...
class ReplayBuffer:
    ...
    def __init__(self, action_size, buffer_size, batch_size, seed):
        ...
        self.experience = namedtuple("Experience", field_names=["state", "action", "reward", "next_state", "abs_td_error", "done"])
        self.seed = random.seed(seed)
    
    def add(self, state, action, reward, next_state, abs_td_error, done):
        """Add a new experience to memory."""
        e = self.experience(state, action, reward, next_state, abs_td_error, done)
    ...
    def sample(self):
        ...
        abs_td_errors = torch.from_numpy(np.vstack([e.abs_td_error for e in experiences if e is not None])).float().to(device)
        dones = torch.from_numpy(np.vstack([e.done for e in experiences if e is not None]).astype(np.uint8)).float().to(device)
        
        return (states, actions, rewards, next_states, abs_td_error, dones)
